...
export default function DrivingSimulator() {
  ...
  const connectWebSocket = useCallback(() => {
    try {
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          if (message.type === "controls") {
            controlsRef.current = message.data
          }
        } catch (error) {
      ...
      console.error("Failed to connect simulator WebSocket:", error)
      setRemoteConnected(false)
    }
  }, []) // Removed isRemoteControlled dependency
  ...
  }, [createParticles, isRemoteControlled]) // Added isRemoteControlled dependency here instead
  ...
  const toggleRemoteControl = () => {
    ...
    if (!isRemoteControlled && !wsRef.current) {
      connectWebSocket()
    }
  }
  ...
  useEffect(() => {
    ...
    return () => {
      if (wsRef.current && !isRemoteControlled) {
        wsRef.current.close()
        wsRef.current = null
      }
    }
  }, [isRemoteControlled]) // Removed connectWebSocket dependency
